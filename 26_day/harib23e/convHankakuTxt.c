// convHankakuTxt.c
// http://bttb.s1.valueserver.jp/wordpress/blog/2017/12/13/makeos-5-1/

#include <stdio.h>

int main() {
    FILE *infp;
    FILE *outfp;
    char s1[] = "char hankaku[4096] =  {\n  ";
    char s2[] = "\n};";
    char buf[256] = {0};
    char wbuf[256] = {0};
    int d;

    if ((infp = fopen("hankaku.txt","r")) == NULL) {
        printf("input file open error\n");
        return 0;
    }

    if ((outfp = fopen("hankaku.c","w")) == NULL) {
        printf("output file open error\n");
        return 0;
    }

    fwrite(s1, sizeof(char), 26, outfp);

    fgets(buf, sizeof(buf), infp);  // 「OSASKの半角フォントを流用」
    for(int k = 0; k < 256; k++) {
        for(int i = 0; i < 2; i++) {
            fgets(buf, sizeof(buf), infp);  // 空行 + 文字コード部分
        }
        for(int i = 0; i < 16; i++) {
            d = 0;
            fgets(buf, sizeof(buf), infp);  // 記号列 -> 16進数
            for(int j = 0; j < 8; j++) {
                if(buf[j] == '*'){d |= (1 << (8-j-1));}
            }
            sprintf(wbuf, "0x%02x, ", d);
            if(k == 255 && i == 15) {
                fwrite(wbuf, 1, 4, outfp);
            } else {
                fwrite(wbuf, 1, 6, outfp);
            }
        }
        fwrite("\n  ", 1, 3, outfp);
    }

    fwrite(s2, sizeof(char), 3, outfp);
    fclose(infp);
    fclose(outfp);
}