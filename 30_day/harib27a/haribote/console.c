...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	task->fat = fat;
	if (japanese[4096] != 0xff) {  /* 日本語フォントファイルを読み込めたか？ */
		task->langmode = 1;
	} else {
		task->langmode = 0;
	}
...
void cons_putstr0(struct CONSOLE *cons, unsigned char *s)
{
	for (; *s != 0; s++) {
...
void cons_putstr1(struct CONSOLE *cons, unsigned char *s, int l)
{
	int i;
	for (i = 0; i < l; i++) {
...
{
	struct SHEET *sheet = cons->sht;
	boxfill8(sheet->buf, sheet->bxsize, COL8_000000, 8, 28, 247, 155);
	sheet_refresh(sheet, 8, 28, 8 + 240, 28 + 128);
	cons->cur_y = 28;
	return;
}
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
			...
			start_app(0x1b, 4, esp, 12, &(task->tss.esp0));
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	if (edx == 1) {
		...
		cons_putstr0(cons, (unsigned char *) ebx + ds_base);
	} else if (edx == 3) {
		cons_putstr1(cons, (unsigned char *) ebx + ds_base, ecx);
	} else if (edx == 4) {
